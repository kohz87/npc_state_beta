...
export const NPC_STATE_VERSION = '0.5.36';
...
// accepted pre-rebase records remain inspectable but cannot claim current-timeline provenance.
...
export function normalizeMemoryEntries(value, max = MEMORY_LIMIT, itemMax = 700) {
    ...
    for (const raw of input) {
        ...
        if (!clean) continue;
        const duplicateIndex = out.findIndex(existing => existing.normalize('NFKC').toLocaleLowerCase().replace(/\s+/g, ' ') === clean.normalize('NFKC').toLocaleLowerCase().replace(/\s+/g, ' '));
        if (duplicateIndex >= 0) {
            continue;
        }
        out.push(clean);
