...
export function buildTargetedRefreshPrompt({ npc, chat, assistantMessageId, scanDepth = 12, memoryCriteria = '', playerName = '', dossierLimits = {} }) {
    ...
    return [
        ...
        'TARGET DOSSIER includes the stored relationshipSummary. Reconcile it as the NPC current relationship dynamic toward the PLAYER: return a new concise natural-language value only when it is missing/invalid or the supplied chat establishes a materially newer dynamic. For a materially newer current-evidence proposal include relationshipSummaryEvidence; repair/reconciliation from already accepted stored relationship context remains separate. This targeted refresh may reconcile relationshipSummary without changing any relationship score. Never copy an output-schema instruction or placeholder into the field; never rewrite merely for style.',
