...
export function buildScanPrompt({ state, chat, assistantMessageId, scanDepth = 2, relationshipCriteria = '', relationshipCaps = DEFAULT_RELATIONSHIP_CAPS, memoryCriteria = '', playerName = '', dossierLimits = {}, admissionMode = 'balanced', relationshipSummaryRepair = false, semanticMode = 'scan', routine = true }) {
    ...
    return [
        ...
        '- apparentAge is separate from actual age. Use ~N when a numeric-looking apparent age is supported. When the source establishes only a visible life-stage descriptor such as child, young woman, middle-aged man, or elderly, preserve one concise grounded descriptor instead of inventing a number. Never convert apparent-age wording into actual chronological age, and never invent a numeric range or decade band.',
...
export function buildTargetedRefreshPrompt({ npc, chat, assistantMessageId, scanDepth = 12, memoryCriteria = '', playerName = '', dossierLimits = {} }) {
    ...
    return [
        ...
        'apparentAge is separate from actual age. Use ~N when numeric appearance-age evidence exists; otherwise preserve one concise grounded visible life-stage descriptor rather than inventing a number. Never convert it into actual age or fabricate a range.',
        ...dossierCollectionRules(limits),
