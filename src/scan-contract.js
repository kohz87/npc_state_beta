...
export function scanOutputExamples({ includeNew = true, includeExisting = true } = {}) {
    ...
    if (includeNew) {
        ...
        const sources = [{ messageId: null, excerpt }];
        populated.exchangeActiveNpcIds.push('Nia');
        populated.inChatNpcIds.push('Nia');
        const semanticUpdates = [
            { field: 'role', operation: 'establish', value: 'Harbor clerk', sources, explanation: 'Visible scene establishes Nia’s current function.' },
            { field: 'background', operation: 'establish', value: 'Clerk of the South Quay Registry.', sources, explanation: 'Visible scene establishes her registry affiliation.' },
            { field: 'apparentAge', operation: 'establish', value: '~20-29', sources, explanation: 'Visible scene describes her as in her twenties.' },
            { field: 'appearance', operation: 'establish', value: 'Blue coat.', sources, explanation: 'Visible scene states the coat.' },
            { field: 'personality', operation: 'establish', value: 'Practical and methodical in registry work.', sources, explanation: 'The scene supports a narrow work-context characterization.' },
            { field: 'behaviorProfile', operation: 'establish', changes: [{ action: 'add', value: 'Guides applicants through forms and checks their entries.' }], sources, explanation: 'The scene shows a reinforcing intake sequence.' },
            { field: 'speech', operation: 'establish', value: 'Brief practical instructions.', sources, explanation: 'Visible dialogue is concise and instructional.' },
            { field: 'status', operation: 'establish', value: 'Processing Ari’s registry form.', sources, explanation: 'Visible scene establishes the current activity.' },
        ];
        const nia = {
        ...
        const proposed = new Set(semanticUpdates.map(update => update.field));
...
export function scanOutputContract(options = {}) {
    ...
    return [
        ...
        compact
            ? 'NEW/EXISTING ordinary fields use semanticUpdates; NEW supported blanks use establish. []=string arrays; appearanceForms:[{name,appearance}].'
            : 'NEW and EXISTING ordinary dossier fields use source-cited semanticUpdates. For NEW supported blank fields use establish. Collections use targeted changes; appearanceForms use named-form scope.',
        compact
            ...
            : 'Routine candidateAccounting maps each supplied stable existing NPC id to evaluated|mentioned|inactive|unresolved; it is separate from activity and fieldEvaluations. NEW/EXISTING patches may add evidence-only profileObservations:[{field,observation,concept?,sources:[{messageId,excerpt}],explanation?}] for personality|behaviorProfile|speech|mannerisms. NEW durable mannerisms also require profileEstablishment.mannerisms=explicit|reinforced.',
        compact
            ? 'Evidence={excerpts:[exact quotes],explanation}; identity adds anchor; activity keys=exchangeActive/inChat/worldActive; semantic sources=[{messageId,excerpt}], messageId:null=current.'
            : 'Evidence:{excerpts:[1-3 exact quotes],explanation}; identityEvidence adds anchor. activityEvidence keys:exchangeActive/inChat/worldActive. Identity/activity=current visible. Semantic/observation sources use exact permitted excerpts; messageId:null=current, number=history.',
