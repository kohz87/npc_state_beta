...
function promoteEditorToTopLayer(doc = globalThis.document) {
...
export function createNpcStateUi(adapters = {}) {
        ...
        return `<div id="${SETTINGS_ID}" class="extension_container npc-state-extension npc-state-v3-settings">
          ...
          <div class="inline-drawer-content npc-state-drawer">
            ...
            <details class="npc-state-v3-dossier-evolution"><summary><b>Dossier evolution</b></summary>
              ...
              <div class="npc-state-settings-grid">
                <label class="npc-state-setting-row"><span><b>Important memories</b><small>Durable events and facts retained for future scenes.</small></span><input id="npc_state_v3_limit_memories" class="text_pole npc-state-number" type="number" min="1" max="${DOSSIER_LIMIT_MAXIMUMS.memories}"></label>
                <label class="npc-state-setting-row"><span><b>Key relationships</b><small>Important non-player relationships retained in the dossier.</small></span><input id="npc_state_v3_limit_key_relationships" class="text_pole npc-state-number" type="number" min="1" max="${DOSSIER_LIMIT_MAXIMUMS.keyRelationships}"></label>
                <label class="npc-state-setting-row"><span><b>Mannerisms</b><small>Current recurring gestures, habits, and tells.</small></span><input id="npc_state_v3_limit_mannerisms" class="text_pole npc-state-number" type="number" min="1" max="${DOSSIER_LIMIT_MAXIMUMS.mannerisms}"></label>
                <label class="npc-state-setting-row"><span><b>Behavioral profile</b><small>Current durable behavioral tendencies and patterns.</small></span><input id="npc_state_v3_limit_behavior" class="text_pole npc-state-number" type="number" min="1" max="${DOSSIER_LIMIT_MAXIMUMS.behaviorProfile}"></label>
              </div>
            </details>
    ...
    function bindSettings(panel) {
        ...
        panel.querySelector('#npc_state_v04_birthday_fill')?.addEventListener('change', event => {
            ...
            getSettings().birthdayFillMode = normalizeBirthdayFillMode(value);
        ...
        panel.querySelector('#npc_state_v04_birthday_days')?.addEventListener('change', event => {
            getSettings().birthdayRandomDaysPerMonth = normalizeNumericSetting('birthdayRandomDaysPerMonth', event.target.value);
        ...
        panel.querySelector('#npc_state_v3_scan_depth')?.addEventListener('change', event => {
            getSettings().scanDepth = normalizeNumericSetting('scanDepth', event.target.value); event.target.value = getSettings().scanDepth; persistSettings();
        });
        panel.querySelector('#npc_state_v3_inject_budget')?.addEventListener('change', event => {
            getSettings().injectBudgetTokens = normalizeNumericSetting('injectBudgetTokens', event.target.value); event.target.value = getSettings().injectBudgetTokens; persistSettings(); onSettingsChanged();
        });
