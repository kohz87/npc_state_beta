...
export function createNpcStateUi(adapters = {}) {
    ...
    async function safely(label, task) {
        try { return await task(); }
            ...
            console.error(`[NPC State v0.4.17] ${label} failed safely`, error);
    ...
    function settingsHtml() {
        return `<div id="${SETTINGS_ID}" class="extension_container npc-state-extension npc-state-v3-settings">
          <div class="inline-drawer"><div class="inline-drawer-toggle inline-drawer-header"><b>NPC State <span class="npc-state-version">0.4.17</span></b><div class="inline-drawer-icon fa-solid fa-circle-chevron-down down"></div></div>
          <div class="inline-drawer-content npc-state-drawer">
            <div class="npc-state-intro">v0.4.17 uses foreground embedded capture for normal turns. Exchange participation, in-chat relevance, and explicit off-screen activity are independent signals. Stable v0.3 dossiers can be cloned once into an independent beta sidecar.</div>
    ...
    function renderRoster() {
        ...
        holder.innerHTML = `<small class="npc-state-muted">Persistent NPC State 0.4.17 database · ${active.length} active · ${archived.length} archived</small><div class="npc-state-roster-chips">${rows(active)}${rows(archived)}</div>`;
