...
async function rebaseCurrentChat(relationshipMode = 'preserve', force = false) {
    ...
    try {
        ...
        if (rebasedSafety?.status === 'safe') {
            console.warn('[NPC State v0.4.43] timeline rebase committed, but the follow-up scan failed', error);
            ...
            console.error('[NPC State v0.4.43] timeline rebase failed safely', error);
...
async function initializeFreshFromUi() {
    ...
    try {
        ...
        console.error('[NPC State v0.4.43] fresh recovery initialization failed safely', error);
...
async function startRecoveryFromUi(control) {
    ...
    try {
        ...
        console.error('[NPC State v0.4.43] historical rebuild failed safely', error);
...
async function resumeRecoveryFromUi() {
    ...
    try {
        ...
        console.error('[NPC State v0.4.43] recovery resume failed safely', error);
