...
async function rebaseCurrentChat(force = false) {
    ...
    try {
        ...
        if (rebasedState?.branchSafety?.status === 'safe') {
            console.warn('[NPC State v0.4.10] timeline rebase committed, but the follow-up scan failed', error);
            ...
            console.error('[NPC State v0.4.10] timeline rebase failed safely', error);
