...
- `present` remains the internal v0.3-compatible storage field, but its v0.4.2 meaning is **in chat**: individually relevant NPC participants at exchange end, not everyone physically nearby.
...
Disable the stable NPC State extension while exercising this beta. Stable may remain installed and its settings/data remain untouched. On first load for a chat with no beta sidecar, 0.4.2 clones the stable v0.3 sidecar into a beta-owned sidecar and then diverges independently.
