...
Relationship scoring remains deterministic and separate: caps, gates, inertia, fractional progress, milestones, replay protection, evidence history, and descriptive Current Dynamic safeguards are extension-owned. Current Dynamic evidence is separate from numeric scoring eligibility: a source-owned, correctly targeted descriptive relationshipSummary may establish or materially update at zero scores/zero deltas without creating relationship-change history, while intensity/milestone safeguards still reject unsupported depth. Direct target binding may use canonical names, unambiguous short identity references, and narrator-addressed second person outside quoted dialogue; when identity and interaction are split across a small coherent evidence set, summary validation reuses the already accepted patch identity/activity binding from that same owned operation instead of running a second identity matcher. Contextual bridging may reuse only activity evidence unambiguously owned by that accepted NPC patch; one role-only interaction excerpt claimed by multiple NPC patches is not target authority. Each exact excerpt still resolves against its original permitted source before quote/addressee classification, so unrelated passages, ambiguous roles, or a shortened fragment from another character's quoted dialogue cannot gain authority. The summary explanation is shape-validated model interpretation rather than lexical proof. Nonzero numeric relationship movement continues through its separate axis-evidence grounding path. Explicit repair/reconciliation keeps its established accepted-history authority.
