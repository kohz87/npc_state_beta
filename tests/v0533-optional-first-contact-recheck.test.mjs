...
function firstPayload({ goalEvaluation = null, includeGoal = false } = {}) {
  ...
  return {
    ...
    npcs: [{
      ...
      semanticUpdates: [
        { field: 'role', operation: 'establish', value: 'Guild intake clerk', sources: [{ messageId: 1, excerpt: visible }] },
        { field: 'location', operation: 'establish', value: 'Adventurer Guild Post', sources: [{ messageId: 1, excerpt: visible }] },
        { field: 'status', operation: 'establish', value: 'Handling the station intake backlog.', sources: [{ messageId: 1, excerpt: visible }] },
        ...(includeGoal ? [{ field: 'goal', operation: 'establish', value: 'Close the intake ledger by dusk.', sources: [{ messageId: 1, excerpt: 'TESSA: I need this ledger closed by dusk.' }] }] : []),
        { field: 'personality', operation: 'establish', value: 'Brisk and efficient during guild intake.', durability: 'durable', sources: [{ messageId: 1, excerpt: visible }], explanation: 'Visible intake actions support a narrow work demeanor.' }],
