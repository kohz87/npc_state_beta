...
function engineHarness(state, chat, settingsOverrides = {}, generated = null) {
    ...
    let generations = 0;
    const settings = normalizeSettings({ autoScan: true, ...settingsOverrides });
    const adapters = {
        getContext: () => context,
        getChatKey: () => key,
        ...
        persistSettings: () => {},
        generate: async () => { generations += 1; if (generated == null) throw new Error('No scanner fixture supplied.'); return typeof generated === 'function' ? generated() : JSON.stringify(generated); },
...
test('minimum selected foreground dossier retains the four live continuity values', () => {
    ...
    assert.doesNotMatch(result.prompt, /FIRST-PASS|semanticUpdates|<npc_state_v1>/);
    assert.match(result.prompt, /SAVED NPC CONTINUITY/);
});

test('one dedicated post-response Scan changes all four fields and persists them', async () => {
    const state = liveState();
    ...
    const scan = payload({
        evaluatedGroups: ALL_GROUPS, semanticUpdates,
        activityEvidence: { exchangeActive: { excerpts: ['Sora walks into the market square.'], explanation: 'Sora acts.' }, inChat: { excerpts: ['Sora studies the posted map.'], explanation: 'Sora remains.' } },
        relationshipChange: noRelationshipChange(),
    });
    const h = engineHarness(state, [{ is_user: true, mes: 'Sora, shall we head out?' }, { is_user: false, mes: visible, swipe_id: 0 }], {}, scan);
    ...
    assert.equal(dossier.mood, 'Relieved.'); assert.equal(dossier.location, 'Market square.');
    assert.equal(dossier.goal, 'Learn the city routes.'); assert.equal(dossier.status, 'Studying the posted map.');
    assert.equal(h.persisted().npcs[0].location, 'Market square.');
    assert.equal((await h.reload().loadChat()).npcs[0].status, 'Studying the posted map.');
...
test('duplicate dedicated Scan retry does not replay player relationship state', async () => {
    const state = liveState();
    const visible = 'Lucien keeps his promise to Sora. Sora thanks him and relies on him a little more.';
    const scan = payload({ evaluatedGroups: ALL_GROUPS, semanticUpdates: [], activityEvidence: {
        exchangeActive: { excerpts: ['Sora thanks him and relies on him a little more.'], explanation: 'Sora reacts.' },
        inChat: { excerpts: ['Sora thanks him and relies on him a little more.'], explanation: 'Sora remains.' },
    }, relationshipChange: { evaluated: true, impact: 'ordinary', delta: { trust: 1, affection: 0, desire: 0, tension: 0 }, priority: ['trust'], axisEvidence: { trust: { excerpts: ['Sora thanks him and relies on him a little more.'], explanation: 'Reliance increases trust.' } }, evidence: 'Sora relies on Lucien more.', reason: 'A kept promise modestly increases trust.' } });
    const h = engineHarness(state, [{ is_user: true, mes: 'I kept my promise.' }, { is_user: false, mes: visible, swipe_id: 0 }], {}, scan);
    await h.engine.loadChat();
    const first = await h.engine.scan(1, { manual: true }); assert.equal(first.ok, true);
    const afterFirst = h.engine.getDossierNpc('npc-sora');
    const snapshot = structuredClone({ relationship: afterFirst.relationship, relationshipProgress: afterFirst.relationshipProgress, relationshipHistory: afterFirst.relationshipHistory, relationshipEvidenceHistory: afterFirst.relationshipEvidenceHistory });
    const second = await h.engine.scan(1, { manual: true, force: true }); assert.equal(second.ok, true);
    const afterSecond = h.engine.getDossierNpc('npc-sora');
    assert.deepEqual({ relationship: afterSecond.relationship, relationshipProgress: afterSecond.relationshipProgress, relationshipHistory: afterSecond.relationshipHistory, relationshipEvidenceHistory: afterSecond.relationshipEvidenceHistory }, snapshot);
});
